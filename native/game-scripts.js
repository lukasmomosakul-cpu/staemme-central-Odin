...
(function () {
  ...
  // Tampermonkey-compatible basics used by many scripts.
  try {
    ...
    window.GM_listValues = window.GM_listValues || function () {
      try {
        return Object.keys(localStorage).filter(function (k) { return k.indexOf('__odin_gm_') === 0; }).map(function (k) { return k.slice(11); });
      } catch (_) { return []; }
    };
    ...
    window.GM_notification = window.GM_notification || function (details) {
      try { console.info('[Odin] GM_notification', details); } catch (_) {}
    };
  } catch (_) {}

  // Managed scripts must only execute on a real in-world page.
