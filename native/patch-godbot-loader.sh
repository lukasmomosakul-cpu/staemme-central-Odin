...
new = r''' private void loadEnabledScripts(WebView v){
  ...
  if(!path.matches("(?i).*die-staemme[.]de.*"))return;
  try{
   ...
   String runner="(function(){try{var CODE="+q+";var fn=new Function(CODE);fn();console.log('ODIN_TEST_USERSCRIPT_EVAL_OK');return 'injected'}catch(e){console.error('ODIN_TEST_USERSCRIPT_EVAL_ERROR',e);return 'error:'+(e&&e.stack?e.stack:(e&&e.message?e.message:String(e)))}})()";
