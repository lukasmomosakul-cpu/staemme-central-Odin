...
new=f''' private void loadEnabledScripts(WebView v){{
 ...
 private void executeGodBotWhenReady(WebView v){{
  String u=v.getUrl()==null?"":v.getUrl(); if(!u.matches("(?i).*[/]game[.]php(?:[?].*)?$"))return;
  // Bei aktiver Zugangssperre wird bewusst NICHTS injiziert: die Captcha-Seite
  // soll unveraendert bleiben, damit sie von Hand geloest werden kann.
  // Marker aus dem echten Seitenquelltext: <div class="quest" id="botprotection_quest">
 ...
 private void injectGodBot(WebView v){{
  try{{if(v.getTag(0x0D1A0002)!=null)return;v.setTag(0x0D1A0002,Boolean.TRUE);
   new Thread(()->{{
    String src=null;
    // Immer zuerst die Live-Fassung aus dem Gist: so wirkt ein Gist-Update
