...
new=r''' private void loadEnabledScripts(WebView v){
  ...
  try{
   ...
   String js="try{new Function("+q+")();var e=document.getElementById('__odin_loader_status');if(e)e.textContent='ODIN 1.1.43 · GodBot eval OK';console.log('ODIN_GODBOT_EVAL_OK')}catch(e){var msg=e&&e.stack?e.stack:(e&&e.message?e.message:String(e));console.error('ODIN_GODBOT_EVAL_ERROR',msg);var x=document.getElementById('__odin_loader_status');if(x){x.innerHTML='ODIN 1.1.43 · GodBot FEHLER<br><pre id=\'__odin_error_text\' style=\'white-space:pre-wrap;user-select:text;color:#f55;margin:5px 0;font:12px monospace\'>ERROR</pre>';var z=document.getElementById('__odin_error_text');z.textContent=msg}}";
