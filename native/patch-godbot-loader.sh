...
# GodBot exposes live page globals with Object.defineProperty(). Android WebView can already
# have non-configurable properties such as Timing. One such collision aborts the entire
# userscript before the actual bot code starts, so make those bridge definitions non-fatal.
python3 - <<'PY'
...
new=r''' private void loadEnabledScripts(WebView v){
  ...
  try{
   ...
   String shim="(function(){if(window.__odinGMShim)return;window.__odinGMShim=true;var p='odin_gm_';var listeners={};function g(k,d){try{var x=localStorage.getItem(p+k);return x===null?d:JSON.parse(x)}catch(e){return d}}function s(k,v){try{var old=g(k,undefined);localStorage.setItem(p+k,JSON.stringify(v));(listeners[k]||[]).forEach(function(f){try{f(k,old,v,false)}catch(e){}})}catch(e){}}function del(k){try{localStorage.removeItem(p+k)}catch(e){}}window.GM_getValue=window.GM_getValue||function(k,d){return g(k,d)};window.GM_setValue=window.GM_setValue||function(k,v){s(k,v)};window.GM_deleteValue=window.GM_deleteValue||function(k){del(k)};window.GM_listValues=window.GM_listValues||function(){var a=[];try{for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k&&k.indexOf(p)===0)a.push(k.substring(p.length))}}catch(e){}return a};window.GM_addValueChangeListener=window.GM_addValueChangeListener||function(k,f){(listeners[k]||(listeners[k]=[])).push(f);return (listeners[k].length-1)};window.GM_removeValueChangeListener=window.GM_removeValueChangeListener||function(){};window.GM_addStyle=window.GM_addStyle||function(c){var x=document.createElement('style');x.textContent=c;(document.head||document.documentElement).appendChild(x);return x};window.GM_registerMenuCommand=window.GM_registerMenuCommand||function(){return 0};window.GM_unregisterMenuCommand=window.GM_unregisterMenuCommand||function(){};window.GM_openInTab=window.GM_openInTab||function(u){try{window.open(u,'_blank')}catch(e){location.href=u}};window.GM_notification=window.GM_notification||function(o){try{console.log(typeof o==='string'?o:(o&&o.text)||'')}catch(e){}};window.GM_getResourceText=window.GM_getResourceText||function(){return ''};window.GM_getResourceURL=window.GM_getResourceURL||function(){return ''};window.unsafeWindow=window.unsafeWindow||window;window.GM_info=window.GM_info||{script:{name:'GodBot',version:'526'},version:'526'};window.GM_xmlhttpRequest=window.GM_xmlhttpRequest||function(o){var x={readyState:4,status:0,responseText:'',response:'',finalUrl:o.url,abort:function(){}};try{var body=OdinNative.httpGet(String(o.url));x.status=200;x.responseText=body;x.response=body;if(o.onload)o.onload(x)}catch(e){x.status=0;if(o.onerror)o.onerror(x)}return x};window.GM=window.GM||{};window.GM.getValue=window.GM.getValue||function(k,d){return Promise.resolve(GM_getValue(k,d))};window.GM.setValue=window.GM.setValue||function(k,v){GM_setValue(k,v);return Promise.resolve()};window.GM.deleteValue=window.GM.deleteValue||function(k){GM_deleteValue(k);return Promise.resolve()};window.GM.listValues=window.GM.listValues||function(){return Promise.resolve(GM_listValues())};window.GM.addStyle=window.GM.addStyle||function(c){return Promise.resolve(GM_addStyle(c))};window.GM.registerMenuCommand=window.GM.registerMenuCommand||function(){return Promise.resolve(0)};window.GM.openInTab=window.GM.openInTab||function(u){return Promise.resolve(GM_openInTab(u))};window.GM.xmlHttpRequest=window.GM.xmlHttpRequest||function(o){return Promise.resolve(GM_xmlhttpRequest(o))};})();";
   final String qShim=JSONObject.quote(shim), qSrc=JSONObject.quote(src); final java.util.ArrayList<String> reqs=new java.util.ArrayList<>(requires);
   new Thread(()->{try{
     java.util.ArrayList<String> depSources=new java.util.ArrayList<>(); java.util.HashSet<String> seen=new java.util.HashSet<>();
     for(String req:reqs){String key=req.replaceFirst("[?#].*$","");if(!seen.add(key))continue;try{HttpURLConnection c=(HttpURLConnection)new URL(req).openConnection();c.setConnectTimeout(15000);c.setReadTimeout(30000);c.setRequestProperty("User-Agent","Mozilla/5.0 (Android) Odin");int st=c.getResponseCode();if(st<200||st>=300)continue;BufferedReader rr=new BufferedReader(new InputStreamReader(c.getInputStream()));String x;StringBuilder d=new StringBuilder();while((x=rr.readLine())!=null)d.append(x).append('\\n');rr.close();depSources.add(d.toString());}catch(Exception ignored){}}
     StringBuilder deps=new StringBuilder("[");for(int i=0;i<depSources.size();i++){if(i>0)deps.append(',');deps.append(JSONObject.quote(depSources.get(i)));}deps.append(']');final String qDeps=deps.toString();
     runOnUiThread(()->{String js="try{new Function("+qShim+")();var __odinDeps="+qDeps+";for(var __i=0;__i<__odinDeps.length;__i++){try{(0,eval)(__odinDeps[__i])}catch(__e){console.error('ODIN_GODBOT_REQUIRE_ERROR',__i,__e&&__e.stack?__e.stack:__e)}}(0,eval)("+qSrc+");var e=document.getElementById('__odin_loader_status');if(e)e.textContent='ODIN 1.1.48 · GodBot eval OK';console.log('ODIN_GODBOT_EVAL_OK')}catch(e){var msg=e&&e.stack?e.stack:(e&&e.message?e.message:String(e));console.error('ODIN_GODBOT_EVAL_ERROR',msg);var x=document.getElementById('__odin_loader_status');if(x){x.innerHTML='ODIN 1.1.48 · GodBot FEHLER<br><pre id=\'__odin_error_text\' style=\'white-space:pre-wrap;user-select:text;color:#f55;margin:5px 0;font:12px monospace\'>ERROR</pre>';var z=document.getElementById('__odin_error_text');z.textContent=msg}}";v.evaluateJavascript(js,null);});
...
p.write_text(s)
main=p.parent/'MainActivity.java'
if main.exists():
 ms=main.read_text();ms=ms.replace('https://github.com/lukasmomosakul-cpu/staemme-central-Odin/releases/latest/download/odin-latest.apk?odin=','https://github.com/lukasmomosakul-cpu/staemme-central-Odin/releases/download/v1.1.48/odin-latest.apk?odin=');ms=ms.replace('"odin-update.apk"','"odin-update-1.1.48.apk"');main.write_text(ms)
PY