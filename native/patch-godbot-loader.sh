...
new=r''' private void loadEnabledScripts(WebView v){
  ...
  try{
   ...
   String js="(function(){try{if(window.__odinGodBotLoaded)return;window.__odinGodBotLoaded=true;new Function("+q+")();console.log('ODIN_GODBOT_EVAL_OK');var e=document.getElementById('__odin_loader_status');if(e)e.textContent='ODIN 1.1.42 · GodBot eval OK'}catch(e){window.__odinGodBotLoaded=false;var msg=e&&e.stack?e.stack:(e&&e.message?e.message:String(e));window.__odinGodBotError=msg;console.error('ODIN_GODBOT_EVAL_ERROR',msg);var x=document.getElementById('__odin_loader_status');if(x){x.innerHTML='ODIN 1.1.42 · GodBot FEHLER<br><pre id="__odin_error_text" style="white-space:pre-wrap;user-select:text;color:#f55;margin:5px 0;font:12px monospace">'+String(msg).replace(/&/g,'&amp;').replace(/</g,'&lt;')+'</pre><button id="__odin_copy_error" style="font:bold 13px sans-serif;padding:7px 10px;margin-top:4px">FEHLER KOPIEREN</button>';x.style.color='#fff';x.style.borderColor='#f55';var c=document.getElementById('__odin_copy_error');c.onclick=function(){var t='ODIN 1.1.42\n'+msg;navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(t).then(function(){c.textContent='KOPIERT ✓'}).catch(function(){prompt('Fehler kopieren:',t)}):prompt('Fehler kopieren:',t)}}}})()";
