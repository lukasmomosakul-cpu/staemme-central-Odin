 ...
 private void injectGodBot(WebView v){{
  try{{
   ...
   new Thread(()->{{try{{
    ...
    runOnUiThread(()->{{String js="try{{new Function("+qShim+")();var d="+qDeps+";for(var i=0;i<d.length;i++){{try{{(0,eval)(d[i])}}catch(e){{console.error('ODIN_GODBOT_REQUIRE_ERROR',e&&e.stack?e.stack:e)}}}}(0,eval)("+qSrc+");console.log('ODIN_GODBOT_EVAL_OK')}}catch(e){{var msg=e&&e.stack?e.stack:(e&&e.message?e.message:String(e));console.error('ODIN_GODBOT_EVAL_ERROR',msg)}}";v.evaluateJavascript(js,null);}});
