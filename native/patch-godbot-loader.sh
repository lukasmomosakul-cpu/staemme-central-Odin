...
if requires:
    ...
    for u in requires:
        try:
            with urllib.request.urlopen(u,timeout=30) as r: parts.append(r.read().decode('utf-8'))
        except Exception as e: raise SystemExit(f'Failed to download @require {u}: {e}')
    asset.write_text('\n'.join(parts)+'\n'+src)
...
new=r''' private void loadEnabledScripts(WebView v){
  ...
  try{
   String test="(function(){try{var e=document.getElementById('__odin_userscript_test');if(!e){e=document.createElement('div');e.id='__odin_userscript_test';e.style.cssText='position:fixed;top:55px;left:50%;transform:translateX(-50%);z-index:2147483647;background:#111;color:#0f0;padding:8px 14px;border:2px solid #0f0;border-radius:6px;font:bold 16px sans-serif;box-shadow:0 2px 8px #000;';(document.body||document.documentElement).appendChild(e)}e.textContent='USERSCRIPT OK';console.log('ODIN_TEST_USERSCRIPT_OK')}catch(x){console.error('ODIN_TEST_ERROR',x)}})()";
   v.evaluateJavascript(test,null);
   String status="(function(){try{var e=document.getElementById('__odin_loader_status');if(!e){e=document.createElement('div');e.id='__odin_loader_status';e.style.cssText='position:fixed;bottom:8px;left:8px;z-index:2147483647;background:#111;color:#fff;padding:5px 9px;border:1px solid #777;border-radius:4px;font:12px sans-serif;opacity:.9;max-width:92vw;white-space:normal;word-break:break-word';(document.body||document.documentElement).appendChild(e)}e.textContent='ODIN 1.1.40 · Loader aktiv'}catch(x){}})()";
   v.evaluateJavascript(status,null);
   BufferedReader r=new BufferedReader(new InputStreamReader(getAssets().open("godbot.user.js")));
   StringBuilder bld=new StringBuilder(); String line;
   while((line=r.readLine())!=null)bld.append(line).append(System.lineSeparator()); r.close();
   String src=bld.toString(); if(src.trim().isEmpty()) return;
   ...
   String js="(function(){try{if(window.__odinGodBotLoaded)return;window.__odinGodBotLoaded=true;new Function("+q+")();console.log('ODIN_GODBOT_EVAL_OK');var e=document.getElementById('__odin_loader_status');if(e)e.textContent='ODIN 1.1.40 · GodBot eval OK'}catch(e){window.__odinGodBotLoaded=false;var msg=e&&e.stack?e.stack:(e&&e.message?e.message:String(e));console.error('ODIN_GODBOT_EVAL_ERROR',msg);var x=document.getElementById('__odin_loader_status');if(x){x.textContent='ODIN 1.1.40 · GodBot FEHLER: '+msg.substring(0,220);x.style.color='#f55';x.style.borderColor='#f55'}}})()";
