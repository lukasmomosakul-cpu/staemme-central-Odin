...
function GamePageContent() {
  ...
  const [gameUrl, setGameUrl] = useState(DEFAULT_GAME_URL), [loading, setLoading] = useState(true), [error, setError] = useState('');
  useEffect(() => { if (!supabase) { setLoading(false); return; } let mounted=true; (async()=>{ setLoading(true); const {data:sessionData}=await supabase.auth.getSession(); const user=sessionData.session?.user; if(!mounted)return; setEmail(user?.email??null); if(!user){setLoading(false);return;} const {data:membership,error:membershipError}=await supabase.from('team_members').select('team_id').eq('user_id',user.id).limit(1).maybeSingle(); if(membershipError||!membership){setError('Kein Team gefunden.');setLoading(false);return;} const {data:rows,error:accountError}=await supabase.from('game_accounts').select('id,name,world,created_at').eq('team_id',membership.team_id).order('created_at',{ascending:false}); if(accountError){setError(accountError.message);setLoading(false);return;} const list=(rows??[]) as Account[]; setAccounts(list); const stored=typeof window!=='undefined'?localStorage.getItem(STORAGE_KEY):null; const initial=requestedAccount&&list.some(a=>a.id===requestedAccount)?requestedAccount:(stored&&list.some(a=>a.id===stored)?stored:(list[0]?.id??'')); setSelectedId(initial); setLoading(false); })(); return()=>{mounted=false}; },[requestedAccount]);
  useEffect(()=>{if(selectedId&&typeof window!=='undefined')localStorage.setItem(STORAGE_KEY,selectedId)},[selectedId]);
