...
type ScriptEntry = { id:string; name:string; source:string; enabled:boolean; type:'Tampermonkey'|'Gist'; code:string; };
...
const defaultScript=():ScriptEntry => ({ id:crypto.randomUUID(), name:'GodBot', source:GODBOT_URL, enabled:true, type:'Gist', code:'' });
...
export default function ScriptsPage(){
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      const {data:rows}=await supabase.from('scripts').select('id,name,source_url,enabled,type,code').eq('team_id',membership.team_id).order('created_at',{ascending:true});
      if(cancelled)return;
      if(rows&&rows.length){
        const mapped=rows.map((r:any)=>({id:r.id,name:r.name,source:r.source_url,enabled:!!r.enabled,type:r.type==='Gist'?'Gist':'Tampermonkey',code:r.code||''} as ScriptEntry));
        setScripts(mapped);
        ...
        const next=inserted?[{id:inserted.id,name:inserted.name,source:inserted.source_url,enabled:inserted.enabled,type:'Gist' as const,code:''}]:[seed];
  ...
  const addCode=async()=>{
    ...
    try{
      const entry:ScriptEntry={id:crypto.randomUUID(),name:titel,source:'inline',enabled:false,type:'Tampermonkey',code:quelle};
      let saved=entry;
      if(supabase){
        const {data:{session}}=await supabase.auth.getSession();
        const user=session?.user;
        ...
        if(membership){
          const {data:inserted,error}=await supabase.from('scripts').insert({id:entry.id,team_id:membership.team_id,name:entry.name,type:entry.type,source_url:'inline',download_url:'inline',code:quelle,enabled:false,created_by:user!.id}).select('id,name,source_url,enabled,type,code').single();
          if(error)throw error;
          saved={id:inserted.id,name:inserted.name,source:inserted.source_url,enabled:inserted.enabled,type:'Tampermonkey',code:inserted.code||''};
        }
      }
  ...
  const add=async()=>{
    ...
    try{
      ...
      const entry={id:crypto.randomUUID(),name:name.trim()||'Tampermonkey Script',source,enabled:false,type:'Tampermonkey' as const,code:''};
      let saved=entry;
      if(supabase){
        const {data:{session}}=await supabase.auth.getSession();
        const user=session?.user;
        ...
        if(membership){
          ...
          if(error)throw error;
          saved={id:inserted.id,name:inserted.name,source:inserted.source_url,enabled:inserted.enabled,type:'Tampermonkey',code:''};
        }
      }
