...
// GodBot ist fest in die App eingebaut. Alte Gist-Eintraege werden hier
// ausgeblendet und von der App uebergangen.
const istGodBot=(r:{source_url?:string|null})=>String(r.source_url||'').includes('GodBot.user.js');
const godbotVersion=():string=>{try{return (window as any).Android?.godbotVersion?.()||'';}catch{return '';}};
...
export default function ScriptsPage(){
  ...
  const [gbVersion,setGbVersion]=useState('');
  useEffect(()=>{setGbVersion(godbotVersion());},[]);

  useEffect(()=>{
    let cancelled=false;
    const load=async()=>{
      if(!supabase){setScripts([]);setReady(true);return;}
      const {data:{session}}=await supabase.auth.getSession();
      const user=session?.user;
      if(!user){setScripts([]);setReady(true);return;}
      const {data:membership}=await supabase.from('team_members').select('team_id').eq('user_id',user.id).limit(1).maybeSingle();
      if(!membership){setScripts([]);setReady(true);return;}
      ...
      if(cancelled)return;
      const mapped=(rows??[]).filter((r:any)=>!istGodBot(r)).map((r:any)=>({id:r.id,name:r.name,source:r.source_url,enabled:!!r.enabled,type:r.type==='Gist'?'Gist':'Tampermonkey',code:r.code||'',version:r.version||''} as ScriptEntry));
      setScripts(mapped);
